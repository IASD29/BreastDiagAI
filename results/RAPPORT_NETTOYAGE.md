...
| Fichier | Contenu | Dossier GitHub recommandé |
|---|---|---|
| `clean_data.py` | Script de nettoyage complet | `src/` |
| `cleaning_report.log` | Journal détaillé de l'exécution | `results/` |
| `breast_cancer_cleaned.csv` | Dataset nettoyé et optimisé | `data/processed/` (si assez léger) sinon non inclus |
| `RAPPORT_NETTOYAGE.md` (ce fichier) | Synthèse lisible du processus | racine du repo ou `results/` |

---

## 5. Comment reproduire ce nettoyage

Dans un terminal (Git Bash ou terminal intégré VS Code), depuis le dossier contenant le script et le CSV :

```bash
python clean_data.py --input "breast_cancer_40_features_1M.csv" --output "breast_cancer_cleaned.csv"
```

Cela génère automatiquement `breast_cancer_cleaned.csv` et `cleaning_report.log`.

---

## 6. Mise en ligne sur GitHub — étapes détaillées

Le dépôt du projet est : `https://github.com/IASD29/BreastDiagAI`

### Étape 1 — Uploader le script de nettoyage
1. Aller sur le repo GitHub
2. Cliquer sur le dossier **`src`**
3. Cliquer sur **"Add file" → "Upload files"**
4. Glisser le fichier `clean_data.py`
5. En bas de page, écrire un message de commit (ex : `Add data cleaning script`)
6. Cliquer sur le bouton vert **"Commit changes"**

### Étape 2 — Uploader le rapport de log
1. Retourner à la racine du repo
2. Cliquer sur le dossier **`results`**
3. **"Add file" → "Upload files"**
4. Glisser `cleaning_report.log`
5. **"Commit changes"**

### Étape 3 — Uploader ce rapport de synthèse (`RAPPORT_NETTOYAGE.md`)
1. Même méthode : dans `results/` (ou à la racine)
2. **"Add file" → "Upload files"**
3. Glisser `RAPPORT_NETTOYAGE.md`
4. **"Commit changes"**

### Étape 4 — Uploader le dataset nettoyé (si sa taille le permet)
⚠️ GitHub limite les uploads web à **25 Mo**. Si `breast_cancer_cleaned.csv` dépasse cette taille malgré l'optimisation mémoire :
- Ne pas l'uploader directement
- Ajouter une note dans `data/processed/README.md` expliquant comment le régénérer avec `clean_data.py`

Si le fichier fait moins de 25 Mo :
1. Créer le dossier `data/processed` (comme pour les autres dossiers, via un premier fichier)
2. **"Add file" → "Upload files"**
3. Glisser `breast_cancer_cleaned.csv`
4. **"Commit changes"**

### Étape 5 — Vérification finale
Retourner sur la page principale du repo et vérifier que la structure ressemble à :
